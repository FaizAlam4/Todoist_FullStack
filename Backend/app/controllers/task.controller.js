...
export const findActiveTaskAll = (req, res) => {
  Task.findAll({where:{projectId:req.query.project_id}})
    .then((data) => {
...
export const createTask = (req, res) => {
  if (req.body.content && req.body.content.trim() == "") {
    ...
    Task.create({ ...req.body, projectId: req.query.project_id })
      .then((data) => {
        res
          .status(200)
...
export const deleteTask = (req, res) => {
  ...
  Task.destroy({ where: { id: id } })
    ...
    .catch((err) => {
      res.send({ message: `Could not delete the task with id=${id}`,error:err.message });
    });
};
